...
namespace {
    ...
    alloc_init() {
        if (!initialized) {
            ...
            default_malloc = (void*(*)(size_t)) (uintptr_t) dlsym(RTLD_NEXT, "malloc"); 
                xyzzy_check(default_malloc != nullptr);
            default_realloc = (void*(*)(void*,size_t)) (uintptr_t) dlsym(RTLD_NEXT, "realloc"); 
                xyzzy_check(default_realloc != nullptr);
            default_calloc = (void*(*)(size_t,size_t)) (uintptr_t) dlsym(RTLD_NEXT, "calloc"); 
                xyzzy_check(default_calloc != nullptr);
            initialized = true;
        }
    }
}
...
main() {
    ...
    // Sorting Test 2
    {
       ...
       for (int i=0;i<10000;i++) deq1.push_back(&deq1, i);

       for (int i=20000;i>=10000;i--) deq1.push_back(&deq1,i);
       ...
       for (int i=0;i<10000;i++) iter2.inc(&iter2);
       ...
       for(int i = 0; i <= 20001; i++) deq2.push_back(&deq2,i);
        ...
        for (int i = 0; i <= 20001; i++) 
        {
            ...
            {
                printf("not equal at %d: %d %d", i, deq1.at(&deq1, i), deq2.at(&deq2, i));
            }
        }
